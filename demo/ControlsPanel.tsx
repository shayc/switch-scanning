...
import type { ScanMethodKind, Timing } from "./App.tsx";
import classes from "./ControlsPanel.module.css";
import { DurationField } from "./DurationField.tsx";
import { METHOD_META, METHOD_ORDER } from "./methodMeta.ts";

interface ControlsPanelProps {
  methodKind: ScanMethodKind;
  onMethodKind: (kind: ScanMethodKind) => void;
  timing: Timing;
...
export function ControlsPanel({
  ...
  return (
    ...
    >
      ...
      >
        <Radio.Group value={methodKind} onChange={onMethodKind}>
          <Stack gap={4}>
            {METHOD_ORDER.map((kind) => {
              const option = METHOD_META[kind];
              return (
                <Radio.Card
                  key={kind}
                  value={kind}
                  ...
                  aria-describedby={`scan-method-${kind}-description`}
                >
                  ...
                  <Stack gap={2} flex={1}>
                    ...
                    <Text
                      ...
                      size="xs"
                      c="dimmed"
                      id={`scan-method-${kind}-description`}
                    >
                      {option.summary}
                    </Text>
                  </Stack>
      ...
      >
        <Stack gap="xs">
          {(methodKind === "auto" || methodKind === "inverse") && (
            <DurationField
          ...
          {methodKind === "dwell" && (
            <DurationField
              ...
              valueMs={timing.dwellDurationMs}
              minMs={100}
              maxMs={4000}
              range
              onChange={(dwellDurationMs) => onTiming({ dwellDurationMs })}
            />
          )}
          {methodKind === "step" && (
            <Switch
                ...
                <Stack gap="sm">
                    ...
                    <NumberInput
                      ...
                      onChange={(value) => {
                        if (typeof value === "number" && value >= 1) {
                          onTiming({ passes: Math.round(value) });
                        }
                      }}
                    />
                  )}
                  {methodKind === "auto" && (
                    <DurationField
                      ...
                      valueMs={timing.transitionDurationMs}
                      minMs={0}
                      onChange={(transitionDurationMs) =>
                        onTiming({ transitionDurationMs })
                      }
                    />
                  )}
                  {(methodKind === "auto" || methodKind === "inverse") && (
                  ...
                  {methodKind === "step" && timing.repeatEnabled && (
                    <>
                      <DurationField
