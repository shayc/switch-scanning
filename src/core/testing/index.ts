...
/**
 * Convenience helper that builds a scanner with a fresh manual clock, starts
 * recording events, and then attaches a fixture in one call.
 */
export function createTestScanner(
  ...
  // Override both ports even for untyped JavaScript callers: the returned
  // manual clock must be the sole scheduler that drives this test scanner.
  const scanner = createScanner({ ...options, clock, scheduler: undefined });
