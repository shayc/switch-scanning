...
const STYLES: readonly ScanStyle[] = [
  stepScan(),
  stepScan({ repeat: { delayMs: 6, intervalMs: 4 } }),
  autoScan({ intervalMs: 5, loops: 2, transitionTimeMs: 3 }),
  singleSwitchStepScan({ dwellTimeMs: 6 }),
  inverseScan({ intervalMs: 5, loops: 2 }),
];
...
function assertPublicInvariants(
  ...
  if (pending) {
    ...
    if (pending.kind !== "transition" && snapshot.status !== "scanning") {
      fail("style timer ownership invariant");
    }
  }
}
    ...
    for (let step = 0; step < 120; step += 1) {
      ...
      switch (operation) {
        ...
        case "toggle-enabled":
          ...
          break;
        case "style":
          styleIndex = Math.floor(random() * STYLES.length);
          trace.push(`style(${STYLES[styleIndex]!.kind}:${styleIndex})`);
          scanner.setOptions(behavior());
          break;
        case "press": {
