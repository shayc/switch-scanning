...
import { autoScan, dwellScan, stepScan } from "../methods/methods.ts";
...
describe("causal dwell selection", () => {
  it("consumes one navigation token and never autonomously rearms", () => {
    ...
    const scanner = createScanner({
      method: dwellScan({ dwellDurationMs: 100 }),
  ...
  it("keeps auto scanning advancing under frequent no-op tree republication", () => {
    const clock = manualClock();
    const scanner = createScanner({
    ...
    // Republish the identical tree repeatedly, each time well inside one
    // interval. A restart-on-reconcile would reset the advance deadline every
    // 30ms and pin the highlight; preserving it lets the 100ms deadline fire.
    for (let i = 0; i < 5; i += 1) {
      clock.advanceBy(30);
      fixture.setNodes([...TARGETS]);
    }
  ...
  it("does not resume a paused dwell after changing to another method", () => {
    const clock = manualClock();
    const scanner = createScanner({
      method: dwellScan({ dwellDurationMs: 100 }),
      clock,
...
describe("safe configuration and lifecycle", () => {
  it("requires a declared back action for back-only group exits", () => {
    expect(() =>
      createScanner({ method: stepScan(), groupExit: "back-only" }),
