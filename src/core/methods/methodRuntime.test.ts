...
import { autoScan, inverseScan, stepScan } from "./methods.ts";
import { createMethodRuntime } from "./methodRuntime.ts";

describe("method runtime", () => {
  it("executes automatic timing without knowing the scan tree", () => {
    const clock = manualClock();
    const advance = vi.fn();
    const runtime = createMethodRuntime({
      method: autoScan({ intervalMs: 100, passes: 2, firstItemPauseMs: 50 }),
      clock,
      scheduler: clock,
      isScanning: () => true,
      advance,
  ...
  it("owns step-repeat scheduling and release", () => {
    ...
    const repeat = vi.fn();
    const runtime = createMethodRuntime({
      method: stepScan({ repeat: { delayMs: 200, intervalMs: 50 } }),
      clock,
      scheduler: clock,
      isScanning: () => true,
      advance,
