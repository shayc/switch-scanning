...
describe("scan session", () => {
  ...
  it("preserves and clamps position when reconciliation removes the highlight", () => {
    ...
    expect(session.reconcile()).toEqual([
      {
        type: "landed",
        previous: { kind: "target", id: "c" },
        current: { kind: "target", id: "d" },
        label: "D",
      },
    ]);
    expect(session.snapshot("scanning")).toMatchObject({
      highlight: { kind: "target", id: "d" },
