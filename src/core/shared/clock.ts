...
export function manualClock(startAt = 0): ManualClock {
  assertNonNegative(startAt, "initial time");
  let current = startAt;
  let seq = 0;
  ...
  return {
    now: () => current,
    schedule(delayMs, callback) {
      assertNonNegative(delayMs, "scheduled delay");
    ...
    advanceBy(ms) {
      assertNonNegative(ms, "manualClock advanceBy(ms)");
      drainUntil(current + ms);
    },
    advanceTo(time) {
      assertNonNegative(time, "manualClock advanceTo(time)");
      if (time < current) {
        fail("manualClock cannot advance backwards");
      }
      drainUntil(time);
    },
    flush() {
