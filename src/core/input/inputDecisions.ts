...
export function decideDiscreteInput(
  ...
  if (atRest(status)) {
    return startedIn === "startable" && startOn === "input"
      ? "start"
      : "ignore";
  }
  return "perform";
...
type ScanPressDecision = "ignore" | "start" | "perform";
...
export function decideScanPress(
  ...
  if (startedIn === "startable") {
    return atRest(status) && startOn === "input" ? "start" : "ignore";
  }
