...
/** Immutable per-gesture context passed to every {@link GestureSink} callback. */
...
/** Scanner lifecycle eligibility captured once at raw press, before any timing resolves. */
...
/** Raw physical-input commands the scanner drives into the gesture engine. */
...
/** Create the gesture engine that stabilizes raw input into recognized actions for one scanner. */
export function createGestureEngine(deps: {
  ...
  function setSwitches(next: Map<string, NormalizedSwitch>): void {
    ...
    for (const [key, state] of [...sources]) {
      ...
      if (nextDef && switchDefinitionsEquals(nextDef, state.def)) {
        state.def = nextDef;
      } else {
        sources.delete(key);
        cancelSource(state);
    ...
    // A repeat-suppression window belongs to the definition that opened it.
    // Retaining it across a *redefinition* (unlike across pause/resume) would
    // keep a switch dead under its new `ignoreRepeatMs` until the old, longer
    // window expired — so a correction cannot take effect. Drop windows for
    // switches whose definition changed or was removed; leave the rest intact.
    for (const switchId of [...blockedUntil.keys()]) {
      const prevDef = switches.get(switchId);
      const nextDef = next.get(switchId);
      if (!prevDef || !nextDef || !switchDefinitionsEquals(prevDef, nextDef)) {
        blockedUntil.delete(switchId);
      }
    }
    switches = next;
  }
...
function timingEquals(a: ResolvedTiming, b: ResolvedTiming): boolean {
  return (
...
function switchDefinitionsEquals(
  a: NormalizedSwitch,
  b: NormalizedSwitch,
): boolean {
  ...
  if (a.type === "discrete" && b.type === "discrete") {
    return (
      a.action === b.action && a.performOn === b.performOn && timingEquals(a, b)
    );
  }

  if (a.type === "scan" && b.type === "scan") {
    return timingEquals(a, b);
  }
