...
export class ScanRegistry {
  ...
  mountGroup(
    ...
    const cycle = findGroupParentCycle(this.groups, entry, false);
  ...
  touchGroup(): void {
    const cycle = findGroupParentCycle(this.groups, undefined, false);
    if (cycle && isDevelopment()) this.reportParentCycle(cycle);
    if (!isDevelopment()) this.repairParentCycles();
    this.markDirty();
  }
  ...
  private repairParentCycles(): void {
    ...
    let cycle = findGroupParentCycle(this.groups);
    while (cycle) {
      this.reportParentCycle(cycle);
      this.groups.get(cycle[0]!)!.parentIdOverride = null;
      cycle = findGroupParentCycle(this.groups);
    }
  }
}

function findGroupParentCycle(
  ...
  const parentOf = (id: string): string | undefined => {
    ...
    if (respectOverrides && entry.parentIdOverride === null) return undefined;
    const parentId =
      (respectOverrides ? entry.parentIdOverride : undefined) ??
      entry.getOptions().parentId;
    return parentId !== undefined && entries.has(parentId)
      ? parentId
      : undefined;
  };
  return findParentCycle(entries.keys(), parentOf);
}
