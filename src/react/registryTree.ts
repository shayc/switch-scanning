...
/** One live target registration the registry tracks between renders. */
...
/** One live group registration the registry tracks between renders. */
...
interface RegistryTreeDiagnostics {
  ...
  /** Non-fatal configuration warning; the sink dev-gates and deduplicates. */
  warn(code: string, message: string): void;
}

const ROOT_PARENT = Symbol("scan-registry-root");
/** Reserved ID of the synthetic root group that wraps every registered node. */
...
/**
 * Walk parent links from every id and return the first cycle found, or null.
 * `parentOf` returning undefined ends a walk (a root or an unknown parent).
 */
...
function repairParentCycles(
  ...
  let cycle = findParentCycle(parents.keys(), parentOf);
  while (cycle) {
    diagnostics.reportParentCycle(cycle);
    parents.set(cycle[0]!, ROOT_PARENT);
    cycle = findParentCycle(parents.keys(), parentOf);
  }
}

/** Whether an element is disabled via the `disabled` property, `:disabled`, or `aria-disabled`. */
